...
void
recalc_maxd(int silent)
{
	DESC *d;
	int temp1, temp2, sitecntr, found, aflags2;
	char all[10], *tstrtokr, *ptsitebuff, tsitebuff[1001];
	char *progatr;
	dbref aowner2;
...
void
check_silent_reboot(int *silent, int *found)
{
	FILE* f;
	char tsitebuff[1001];
...
static int
poll_activity(struct timeval *timeout)
{
#ifdef TLI
	return poll(fds, maxfds, timeout->tv_sec);
#else
	return select(maxd, &input_set, &output_set, (fd_set*)NULL,
				   timeout);
#endif
}

static int
handle_poll_error(int err)
{
	DESC *d;
	if (errno != EINTR)
	{
...
{
	static int new_connection_error_count = 0;
	int check;
	DESC *newd;
...
check_client_activity()
{
	...
	int check, aflags2;
	CMDENT* cmdp = NULL;
	int i_len, i_cntr, i_progatr, anum;
	dbref aowner2;
	char *progatr, s_cutter[6],
		 s_cutter2[8], *progatr_str, *progatr_strptr, *s_progatr, *b_progatr,
		 *t_progatr, *b_progatrptr;
	unsigned int ulCRC32;
	ATTR* ap;

	static int flagkeep, i_oldlasttime, i_oldlastcnt;
...
shovechars(int port, char* address)
{
	...
	int found = -1;
	int avail_descriptors, maxfds, active_auths;
	int silent = 0;
	...
	door_processInternalDoors();
		...
		found = poll_activity(&timeout);

		if (found < 0)
		{
			if (handle_poll_error(found) == 1)
				break;
		}
