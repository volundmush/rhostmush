...
static int _naws_telnet(telnet_t *telnet, const char* buffer, size_t size) {
    telnet_event_t ev;
    /* NAWS requires at least 4 bytes... */
    if (size < 4) return 0;

    ev.type = TELNET_EV_NAWS;

    ev.naws.width =  ((unsigned char)buffer[0] << 8) | (unsigned char)buffer[1];
    ev.naws.height = ((unsigned char)buffer[2] << 8) | (unsigned char)buffer[3];
